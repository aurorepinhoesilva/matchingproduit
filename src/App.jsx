...
const initialRows = [
  ...
  },
  {
    ...
    sansOffre: false,
    pinnedMarketPublic: true,
    pinnedCatalog: "Lot marché public Fruits & légumes",
  },
  {
    id: 3,
...
const candidatesByAliment = (row) => {
  ...
  const count = (row.id % 7) + 1;
  const sliced = pool.slice(0, count).map((c, i) => ({
    ...c,
  ...
  if (sliced[0]) {
    sliced[0].favori = !!row.pinnedFavori;
...
function MatchPopup({ row, onClose, onConfirm, onViewProduct }) {
  ...
  const filtered = basePool
    .filter((c) => {
      if (c.id === selectedId) return false; // pinned separately, always shown
  ...
  const isFiltering = !!(search || fFournisseur.length || fGamme.length || fOrigine.length || fLabel.length || fTarif.length);
        ...
        <div className="px-6 mt-3 overflow-y-auto flex-1 pb-2 min-h-0">
          <div className="pb-2">
            {!showSuggestions ? (
              ...
              <>
                {pinnedCandidate && (
                  <div className={isFiltering ? "pb-3 mb-2 border-b" : "mb-2"} style={{ borderColor: T.border }}>
                    <CandidateCard
                      c={pinnedCandidate}
                      row={row}
                      selected={true}
        ...
        <div className="flex-1 min-w-0 overflow-auto space-y-5 pb-8 mt-4">
          ...
          <div className="rounded-xl border overflow-hidden" style={{ borderColor: T.border }}>
            ...
            <div className="px-5 py-4">
                ...
                {ingredients.map((ing) =>
                  editingId === ing.id ? (
                    <div
                      ...
                      style={{ gridTemplateColumns: "1fr 140px 80px 90px auto" }}
                    >
                      <div className="relative">
                        <select
                          value={draftAliment}
                          onChange={(e) => setDraftAliment(e.target.value)}
                          className="w-full appearance-none border-2 rounded-lg pl-3 pr-8 py-2 text-[13px] outline-none bg-white"
                          style={{ borderColor: T.teal, color: draftAliment ? "#1f2430" : "#9aa0a6" }}
                        >
                          <option value="">Aliment</option>
                          <option value="Aubergine">Aubergine</option>
                          <option value="Aubergine lamelle">Aubergine lamelle</option>
                          <option value="Aubergine grillée">Aubergine grillée</option>
                          <option value="Aubergine en cubes">Aubergine en cubes</option>
                          <option value="Riste d'aubergines aubergines tomates oignons">
              ...
              {addingNew ? (
                <div className="grid gap-2 items-center mt-3" style={{ gridTemplateColumns: "1fr 140px 80px 90px auto" }}>
                  <div className="relative">
                    <select
                      value={draftAliment}
                      onChange={(e) => setDraftAliment(e.target.value)}
          ...
          <div className="rounded-xl border overflow-hidden" style={{ borderColor: T.border }}>
            <div className="px-5 py-3 flex items-center" style={{ background: "#efefef" }}>
              <div className="flex items-center gap-2">
